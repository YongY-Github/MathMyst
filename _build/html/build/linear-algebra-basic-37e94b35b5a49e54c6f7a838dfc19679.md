...
A **scalar** is a single number, such as income, price, or interest rate.
...
This non-commutativity is economically important: the *order* of transformations matters.

---

## Transpose of a Matrix

The **transpose** of a matrix $A$, denoted $A'$, is obtained by swapping rows and columns:
$$
(A')_{ij} = a_{ji}.
$$
...
The **zero matrix** has all elements equal to zero and acts as the additive identity.
...
If $Ax = b$, then multiplying both sides by $A^{-1}$ gives:
$$
x = A^{-1} b.
$$
...
The **determinant** of a square matrix is a scalar value that summarizes
...
- The determinant reflects the *volume scaling* effect of a linear transformation.
...
Columns of a matrix are **linearly dependent** if one column can be written as
...
The **rank** of a matrix measures the number of linearly independent rows or columns.
